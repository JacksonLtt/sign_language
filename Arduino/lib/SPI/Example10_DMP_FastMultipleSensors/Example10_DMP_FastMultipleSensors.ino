...
unsigned long init_time_accel = micros();
...
void setup()
{
  ...
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Cpass, 0.5) == ICM_20948_Stat_Ok); // Set to 225Hz
...
void loop()
{
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    if ( (data.header & DMP_header_bitmap_Compass) > 0 ) // Check for Compass
    {
      ...
      Serial.println(end_time - init_time_comp);
      init_time_comp = micros();
    }
    ...
    if ((data.header & DMP_header_bitmap_Accel) > 0) // Check for Accel
    {
      ...
      Serial.print("diff accel:");
      Serial.println(end_time - init_time_accel);
      init_time_accel = micros();
    }
  }
  ...
  if ( (data.header & DMP_header_bitmap_Gyro) > 0 ) // Check for Gyro
  {
    ...
    Serial.print("diff gyro:");
    Serial.println(end_time - init_time_gyro);
    init_time_gyro = micros();
  }
