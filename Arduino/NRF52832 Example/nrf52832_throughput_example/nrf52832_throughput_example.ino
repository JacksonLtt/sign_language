...
void test_throughput(void)
{
  ...
  while ( (remaining > 0) && Bluefruit.connected() && bleuart.notifyEnabled() )
  {
    ...
    //    if ( !bleuart.write(dataArray, data_mtu) ) break;
    size_t sent_data_size = bleuart.write(byteArray, sizeof(byteArray));
    Serial.print("sent_data_size: ");
    Serial.println(sent_data_size);
    if ( !sent_data_size ) break;

    remaining = 0;

  }

  print_speed("Sent ", sent, micros() - start);
}
...
void loop(void)
{
  if (Bluefruit.connected() && bleuart.notifyEnabled())
  {
    ...
    // then reset rx count
    if ( (rxCount > 0) && (rxLastTime + 1000 < micros()) )
    {
